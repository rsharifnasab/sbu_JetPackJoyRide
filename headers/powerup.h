...
bool add_power_up()
{
    ...
    if (timer > powerup_rate) {
        ...
        if (power_up.type == Pspeed) {
            power_up.tex = SBDL::loadTexture ("./assets/pic/speedToken/speed token.png");
        }

        if (power_up.type == Pgravity) {
            power_up.tex = SBDL::loadTexture ("./assets/pic/menu/gravity_token.png");
        }
    }
...
bool move_powerup()
{
    if (power_up.x + power_up.tex.width < 0) {
        power_up.type = Pnone;
        return false;
    }
...
bool speed_token_counter()
{
    ...
    if (x > speed_token_len) {
        barry.speed_token = false;
        x= 0;
        power_up.type=Pnone;
    }
