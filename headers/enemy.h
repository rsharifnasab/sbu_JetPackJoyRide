...
int enemy_y_gen()
{
    if (enemy.type == Emissle) {
        return (barry.y+100 - (rand()%250));
    } else if (enemy.type == EzapperV) {
        return (screen_height/10 * (rand()%8));
    } else {
        return (screen_height/15 * (rand()%14));
    }
...
enemy_types enemy_type_rand()
{
    ...
    if (a==0) {
        ...
        return Ecoin;    // not necesary but for avoiding warning
    }
}
...
bool add_enemy()
{
    ...
    if (timer > enemy_rate) {
        ...
        if (enemy.type == Emissle) {
            play_sound (missle_warning_s);
        }

        if (enemy.type == Elaser) {
            play_sound (laser_warning_s);
        }
    }
...
bool move_enemy()
{
    ...
    if (enemy.x + enemy.tex[enemy.type][enemy.this_tex].width < 0) {
        enemy.type = Ecoin;
        return false;
    }
